<?php

/* @var $this yii\web\View */

$this->title = 'ECM2 | Inicio';
?>
<div class="site-index">

    
    
    <div class="row">
        <div class="col-md-12">
            <image alt="ECM2 Modelo de madurez en Ciberseguridad" src="https://pbs.twimg.com/media/DjymHr9V4AAoWwT.jpg" />
        </div>
    </div>


    


    <!-- Marketing messaging and featurettes
    ================================================== -->
    <!-- Wrap the rest of the page in another container to center all the content. -->

    <div class="container marketing">

      <!-- START THE FEATURETTES -->
      
      <h1>Acerca del modelo</h1>

      <hr class="featurette-divider">

      <div class="row featurette">
        <div class="col-md-7">
          <h2 class="featurette-heading">Completo</h2>
          <p class="lead">El modelo debe de contar con un esquema definido. &Aacute;reas de estudio definidas, as&iacute; como tambi&eacute;n con sus escalas definidas y explicadas.</p>
        </div>
        <div class="col-md-5">
          <img class="featurette-image img-responsive center-block" src="https://pbs.twimg.com/media/DbHxi6jU0AAQpAV.jpg" alt="Completo">
        </div>
      </div>

      <hr class="featurette-divider">

      <div class="row featurette">
        <div class="col-md-7 col-md-push-5">
          <h2 class="featurette-heading">F&aacute;cil de <span class="text-muted">implementar.</span></h2>
          <p class="lead">El modelo debe ser sencillo para que una persona con experiencia en TI de la empresa sea capaz de implementar el modelo y hacer la evaluaci&oacute;n de la empresa.</p>
        </div>
        <div class="col-md-5 col-md-pull-7">
            
            <img class="featurette-image img-responsive center-block" src="https://pbs.twimg.com/media/DbHvJJ4VQAAqb1B.jpg" alt="F&aacute;cil de implementar">
        </div>
      </div>

      <hr class="featurette-divider">

      <div class="row featurette">
        <div class="col-md-7">
          <h2 class="featurette-heading">F&aacute;cil <span class="text-muted">de comprender</span></h2>
          <p class="lead">Un modelo de madurez, no se hace para cumplir, se hace para conocer a fondo a la empresa y por eso es vital que las personas encargadas de implementarlo, lo comprendan a cabalidad.</p>
           <p class="lead">Adem&aacute;s, debe ser una herramienta de comunicaci&oacute;n para usar con la alta gerencia y que permita ver de forma clara la situaci&oacute;n real de la empresa, en cuanto a Ciberseguridad en un lenguaje claro, y sin tener que entrar en tecnicismos que pueden complicar la comunicaci&oacute;n.</p>
        </div>
        <div class="col-md-5">
          <img class="featurette-image img-responsive center-block" src="https://pbs.twimg.com/media/DbCp7HYV4AA8_ER.jpg" alt="Facil de comprender">
        </div>
      </div>

      <hr class="featurette-divider">
      
        <div class="row featurette">
        <div class="col-md-7 col-md-push-5">
          <h2 class="featurette-heading">Actualizado</h2>
          <p class="lead">En un ambiente tan cambiante como lo es el mundo de IT, es imperativo que el modelo sea actualizado para que contemple todos los nuevos factores que puedan afectar el modelo.</p>
                 <p class="lead">Otro de los graves problemas es que las implementaciones de modelos propietarios son muy costosas, lo que provoca que algunas empresas medianas no puedan acceder a ellos y estas quedan vulnerables ante los crecientes riesgos en materia de Ciberseguridad.</p>
        </div>
        <div class="col-md-5 col-md-pull-7">
          <img class="featurette-image img-responsive center-block" src="https://pbs.twimg.com/media/DbH2xL9UMAEtYgH.jpg" alt="Actualizado">
        </div>
      </div>

      <!-- /END THE FEATURETTES -->


  

    </div><!-- /.container -->
</div>
